...
export function calculateBlueprintCAInFCFA(challenges: BlueprintChallenge[] = [], month: string): number {
  return (challenges || [])
    ...
    .reduce((sum, c) => {
      const regFee = (c.registrationFee && c.registrationFee !== 10000) ? c.registrationFee : 15000;
      const count = c.paidParticipantsCount !== undefined ? c.paidParticipantsCount : 0;
      const totalAmount = count * regFee;
